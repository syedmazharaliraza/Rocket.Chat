	...
	implements IBusinessHourBehavior
{
	...
	async openBusinessHoursByDayAndHour(day: string, hour: string): Promise<void> {
		const businessHours = await this.BusinessHourRepository.findActiveBusinessHoursToOpen(
			day,
			hour,
			undefined,
			{
	...
	async closeBusinessHoursByDayAndHour(day: string, hour: string): Promise<void> {
		const businessHours = await this.BusinessHourRepository.findActiveBusinessHoursToClose(
			day,
			hour,
			undefined,
			{
	...
	async afterSaveBusinessHours(businessHourData: IBusinessHoursExtraProperties): Promise<void> {
		...
		const toRemove = [
			...(currentDepartments || []).filter(
				(dept: Record<string, any>) => !departments.includes(dept._id),
			),
		];
		await this.removeBusinessHourFromRemovedDepartmentsUsersIfNeeded(
		...
		const businessHourIdToOpen = (await filterBusinessHoursThatMustBeOpened([businessHour])).map(
	...
	private async handleRemoveAgentsFromDepartments(
		...
		for (const agentId of agentsIds) {
			...
			if (
				!(
					await this.DepartmentsAgentsRepository.findAgentsByAgentIdAndBusinessHourId(
						agentId,
		...
		if (department.businessHourId) {
			await removeBusinessHourByAgentIds(
				agentIdsToRemoveCurrentBusinessHour,
				department.businessHourId,
			);
		}
		...
		await this.UsersRepository.addBusinessHourByAgentIds(
			agentIdsWithoutDepartment,
			defaultBusinessHour._id,
		);
		return options;
	}
	...
	private async removeBusinessHourFromRemovedDepartmentsUsersIfNeeded(
		...
		const agentIds = (
			await this.DepartmentsAgentsRepository.findByDepartmentIds(departmentsToRemove).toArray()
